...
const PriceCalculator = () => {
    ...
    const [gstPercentageForTotalCalc, setGstPercentageForTotalCalc] = useState('');
    ...
    const calculateMainPrice = () => {
        if (totalPrice && gstPercentage) {
            const mainPriceValue = (totalPrice / (1 + gstPercentage / 100)).toFixed(2);
    ...
    const calculateTotalPrice = () => {
        if (mainPriceForTotalCalc && gstPercentageForTotalCalc) {
            const gstValue = ((mainPriceForTotalCalc * gstPercentageForTotalCalc) / 100).toFixed(2);
            const totalPriceValue = (parseFloat(mainPriceForTotalCalc) + parseFloat(gstValue) * 2).toFixed(2);
    ...
    return (
        <Container style={styles.container}>
                ...
                <Col>
                    ...
                    <Form>
                        <Form.Group controlId="calcType">
                            ...
                            >
                                <option value="mainToTotal">Main(Without GST) to Total(With GST)</option>
                                <option value="totalToMain">Total(With GST) to Main(Without GST)</option>
                            </Form.Control>
                        </Form.Group>
                    </Form>
                </Col>
            ...
            {calcType === 'totalToMain' ? (
                <Row className="mb-4">
                    <Col>
                        <h3 style={styles.header}>Calculate Main Price from Total Price with GST</h3>
                        <Form>
                            ...
                            <Button variant="primary" onClick={calculateMainPrice} style={styles.button}>
                ...
                <Row>
                    <Col>
                        <h3 style={styles.header}>Calculate Total Price with GST from Main Price</h3>
                        <Form>
                            <Form.Group controlId="mainPriceForTotalCalc" style={styles.formGroup}>
                            ...
                            <Form.Group controlId="gstPercentageForTotalCalc" style={styles.formGroup}>
                                <Form.Label>GST Percentage</Form.Label>
                                <Form.Control
                                    type="number"
                                    placeholder="Enter GST percentage"
                            ...
                            <Button variant="primary" onClick={calculateTotalPrice} style={styles.button}>
